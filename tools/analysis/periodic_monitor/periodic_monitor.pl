#! /usr/bin/perl
    ...
    #   is intended to process a series of tests run in the same
    #   location over time, as can be done automatically by the
    #   "Script: Periodic test" script supplied with Gridmark.
    ...
    #   time and second by the extended test name.  The value
    ...
    #   extended tests (test:subtest), and times from the
    #   transcript.  There is no guarantee that all tests will
    #   have been run at all times.
    my (%tests, %extests, %times);
    ...
    #   time at which the test was run.
        ...
        if ($l =~ m/$logItem/) {
            ...
            if (($r[0] eq "Gm") && ($r[1] == 1) &&
                ...
                if ($testname eq "info") {
                    ...
                    #   We're monitoring a fixed location.  Ignore teleports
                } else {
